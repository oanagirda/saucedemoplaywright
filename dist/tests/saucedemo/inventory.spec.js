...
const test_fixtures_1 = require("../../fixtures/test-fixtures"); // brings in typed fixtures: login, inventory, etc.
/**
 * Inventory test suite
 * Covers sorting by price and cart badge behavior when adding/removing items.
 */
test_fixtures_1.test.describe('inventory', () => {
    /**
     * Verify sorting by "Price: low to high"
     * Ensures the page sorts prices ascending when selecting "lohi" in the dropdown.
     */
    (0, test_fixtures_1.test)('filter Price low->high', async ({ page, login, inventory }) => {
        ...
        (0, test_1.expect)(nums).toEqual(sorted);
    });
    /**
     * Add/remove behavior
     ...
     */
    (0, test_fixtures_1.test)('add/remove toggles button and badge', async ({ login, inventory }) => {
        ...
        await (0, test_1.expect)(inventory.badge).toHaveCount(0);
    });
});
